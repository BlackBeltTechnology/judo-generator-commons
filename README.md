...
```mermaid
flowchart TD
    Start([generateFile called]) --> EvalCond{Evaluate condition expression?}
    ...
    CopyContent --> CheckPerm{Template has permission?}
    SetContent --> CheckPerm
    
    CheckPerm -->|Yes| UseTmplPerm[Use template permission]
    CheckPerm -->|No| CheckGlobalPerm{Model has global permission?}
...
flowchart TD
    ...
    Filter --> InitIgnore[Initialize GeneratorIgnore\n& ChecksumIgnore]
    
    InitIgnore --> CalcNew[Calculate checksums\nfor new files]
    CalcNew --> ReadSaved[Read saved .generated-files]
    ReadSaved --> ReadFS[Read filesystem checksums]
    
    ReadFS --> CreateMaps[Create Map objects:\n- generatorFileMap\n- savedFileMap\n- filesystemFileMap]
    
    CreateMaps --> DeleteOld{Files in filesystem but not in generator?}
    ...
    DeleteFile --> Validate{validateChecksum enabled?}
    KeepFile1 --> Validate
    
    Validate -->|Yes| CheckMismatch{Checksum mismatch in filesystem?}
    Validate -->|No| PrepareWrite
    
    CheckMismatch -->|Yes| CheckCsumIgnore{In .generator-checksum-ignore?}
    CheckMismatch -->|No| PrepareWrite
    
    CheckCsumIgnore -->|No| CheckGenIgnore{In .generator-ignore?}
    CheckCsumIgnore -->|Yes| PrepareWrite
    
    CheckGenIgnore -->|No| ThrowError[Throw IllegalStateException:\nManual changes detected]
    CheckGenIgnore -->|Yes| PrepareWrite[Prepare files to write]
    
    PrepareWrite --> FilterWrite{Need to write?}
    
    FilterWrite -->|Yes| CheckIgnoreWrite{In .generator-ignore?}
    ...
    WriteFile --> SetPerm{Has permissions?}
    ...
    NextFile -->|No| UpdateIndex[Update .generated-files\nwith new checksums]
...
flowchart TD
    ...
    CreateCtx --> SetVars[Set variables:\n- #self\n- #model\n- #actorType]
    
    SetVars --> RegHelpers[Register helper methods\nas SpringEL functions]
    
    RegHelpers --> EvalFactory{Evaluate factoryExpression?}
    ...
    GetPath --> EvalCond{Evaluate conditionExpression?}
    
    EvalCond -->|Yes| GetBool[Get boolean result]
    EvalCond -->|No| BuildHBS
    
    GetBool --> CheckBool{Result == true?}
...
```mermaid
flowchart TD
    Start([Helper method called]) --> InTemplate{Called from template or SpringEL?}
    ...
    GetResult --> UnbindTL{Needs unbind?}
...
flowchart TD
    ...
    ReadSaved --> ParseSaved[Parse saved checksums:\npath,md5hash]
    
    ParseSaved --> ReadFS[Scan filesystem files]
    
    ReadFS --> CalcFS[Calculate MD5 for each file]
    
    CalcFS --> Compare{For each file in saved list}
    
    Compare --> FileExists{File exists in filesystem?}
    
    FileExists -->|No| MissingOK[OK - Will be regenerated]
    FileExists -->|Yes| CompareHash{Saved hash == Filesystem hash?}
    
    CompareHash -->|Yes| MatchOK[OK - No changes]
    CompareHash -->|No| CheckCsumIgnore{In .generator-checksum-ignore?}
    ...
    HasErrors -->|Yes| ThrowException[Throw IllegalStateException\nwith error list]
...
```mermaid
flowchart TD
    Start([Start generation]) --> CheckActorBased{Template has actorTypeBased=true?}
    ...
    FilterActors --> LoopActors{For each actor type}
    
    LoopActors --> SetActorCtx[Set context variables]
    
    SetActorCtx --> EvalGuard{Has context guard?}
    
    EvalGuard -->|Yes| CheckGuard{Guard passes?}
    EvalGuard -->|No| EvalConstraints
    
    CheckGuard -->|No| SkipActor[Skip all constraints for actor]
    CheckGuard -->|Yes| EvalConstraints
    
    EvalConstraints --> LoopTemplates{For each template}
    
    LoopTemplates --> HasTmplGuard{Has template guard?}
    
    HasTmplGuard -->|Yes| CheckTmplGuard{Guard passes?}
    HasTmplGuard -->|No| GenFile
    
    CheckTmplGuard -->|No| SkipTemplate[Skip template]
    CheckTmplGuard -->|Yes| GenFile[Generate file]
    
    GenFile --> ResolveDir[Resolve target directory]
    
    ResolveDir --> AddToResult[Add to GeneratorResult per actor]
    ...
    WriteActorFiles --> LoopActorResults{For each actor result}
    
    LoopActorResults --> WriteActorDir[Write to actor-specific directory]
    
    WriteActorDir --> NextActorResult{More actor results?}
    
    NextActorResult -->|Yes| LoopActorResults
    NextActorResult -->|No| End
    
    NormalFlow --> WriteCommon[Write to common directory:\n.generated-files]
