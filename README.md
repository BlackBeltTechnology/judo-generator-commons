...
```mermaid
flowchart TD
    Start([generateFile called]) --> EvalCond{Evaluate\ncondition\nexpression?}
    ...
    CopyContent --> CheckPerm{Template has\npermission?}
    SetContent --> CheckPerm
    
    CheckPerm -->|Yes| UseTmplPerm[Use template permission]
    CheckPerm -->|No| CheckGlobalPerm{Model has\nglobal permission?}
...
flowchart TD
    ...
    CreateMaps --> DeleteOld{Files in filesystem\nbut not in generator?}
    ...
    DeleteFile --> Validate{validateChecksum\nenabled?}
    KeepFile1 --> Validate
    
    Validate -->|Yes| CheckMismatch{Checksum\nmismatch in\nfilesystem?}
    Validate -->|No| PrepareWrite
    
    CheckMismatch -->|Yes| CheckCsumIgnore{In .generator-\nchecksum-ignore?}
    CheckMismatch -->|No| PrepareWrite
    
    CheckCsumIgnore -->|No| CheckGenIgnore{In .generator-\nignore?}
    ...
    FilterWrite -->|Yes| CheckIgnoreWrite{In .generator-\nignore?}
...
flowchart TD
    ...
    RegHelpers --> EvalFactory{Evaluate\nfactoryExpression?}
...
```mermaid
flowchart TD
    Start([Helper method called]) --> InTemplate{Called from\ntemplate or\nSpringEL?}
    ...
    FindHelper --> CheckCache{Method in\ncache?}
...
flowchart TD
    ...
    CalcFS --> Compare{For each file\nin saved list}
    
    Compare --> FileExists{File exists\nin filesystem?}
    
    FileExists -->|No| MissingOK[OK - Will be regenerated]
    FileExists -->|Yes| CompareHash{Saved hash ==\nFilesystem hash?}
    
    CompareHash -->|Yes| MatchOK[OK - No changes]
    CompareHash -->|No| CheckCsumIgnore{In .generator-\nchecksum-ignore?}
...
```mermaid
flowchart TD
    Start([Start generation]) --> CheckActorBased{Template has\nactorTypeBased=true?}
    ...
    FilterActors --> LoopActors{For each\nactor type}
    
    LoopActors --> SetActorCtx[Set context:\n- #actorType = current actor\n- #self = current actor]
    
    SetActorCtx --> EvalGuard{Has context\nguard?}
    
    EvalGuard -->|Yes| CheckGuard{Guard\npasses?}
    EvalGuard -->|No| EvalConstraints
    
    CheckGuard -->|No| SkipActor[Skip all constraints for actor]
    CheckGuard -->|Yes| EvalConstraints
    
    EvalConstraints --> LoopTemplates{For each\ntemplate}
    
    LoopTemplates --> HasTmplGuard{Has template\nguard?}
    
    HasTmplGuard -->|Yes| CheckTmplGuard{Guard\npasses?}
    HasTmplGuard -->|No| GenFile
    
    CheckTmplGuard -->|No| SkipTemplate[Skip template]
    CheckTmplGuard -->|Yes| GenFile[Generate file]
    
    GenFile --> ResolveDir[Resolve target directory:\nactorTypeTargetDirectoryResolver]
    
    ResolveDir --> AddToResult[Add to GeneratorResult\ngeneratedByDiscriminator[actor]]
    ...
    WriteActorFiles --> LoopActorResults{For each actor\nresult}
    
    LoopActorResults --> WriteActorDir[Write to actor directory:\n.generated-files-[actorName]]
    
    WriteActorDir --> NextActorResult{More actor\nresults?}
