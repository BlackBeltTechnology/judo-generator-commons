...
| `generateToDirectory()`, `writeDirectory()`, `generateFile()`

| `ModelGeneratorContext`
| State management and engine configuration
| `createHandlebars()`, `createSpringEvaluationContext()`

| `GeneratorModel`
| Template collection loaded from YAML
| `loadYamlURL()`, `overrideTemplates()`

| `GeneratorTemplate`
| Single template configuration with expressions
| `evalToContextBuilder()`

| `TemplateEvaluator`
| Evaluates SpringEL expressions and applies templates
| `getFactoryExpressionResultOrValue()`
...
| Maintains .gitignore with generated files
| `addGeneratedFiles()`
|===
