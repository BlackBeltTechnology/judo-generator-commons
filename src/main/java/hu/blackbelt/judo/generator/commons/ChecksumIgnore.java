...
/**
 * It represents the checksum ignore. This is used to ignore checksum errors for defined patterns.
 */
@Slf4j
public class ChecksumIgnore {
    public static final String GENERATOR_IGNORE_FILE = ".generator-checksum-ignore";
    ...
    CacheLoader<Path, List<String>> loader = new CacheLoader<Path, List<String>>() {
        ...
        public List<String> load(Path key) {
            if (key.resolve(GENERATOR_IGNORE_FILE).toFile().exists()) {
                try {
                    List<String> globs = Files.readAllLines(key.resolve(ChecksumIgnore.GENERATOR_IGNORE_FILE), StandardCharsets.UTF_8);
                    return globs;
                } catch (IOException e) {
    ...
    public ChecksumIgnore(Path rootPath) {
